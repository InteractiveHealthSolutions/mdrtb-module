...
public class TB03uForm extends AbstractSimpleForm {
	...
	public TB03uForm(Patient patient) {
		super(patient);
		this.encounter.setEncounterType(Context.getEncounterService().getEncounterType("TB03 - MDR"));		
	}
	...
	public String getSldRegisterNumber() {
		Obs obs = MdrtbUtil.getObsFromEncounter(Context.getService(MdrtbService.class).getConcept(TbConcepts.SLD_REGISTRATION_NUMBER), encounter);
	...
	public void setSldRegisterNumber(String number) {
		...
		if (obs == null || obs.getValueText() == null || !obs.getValueText().equals(number)) {
			...
			// now create the new Obs and add it to the encounter	
			if(number != null) {
				obs = new Obs (encounter.getPatient(), Context.getService(MdrtbService.class).getConcept(TbConcepts.SLD_REGISTRATION_NUMBER), encounter.getEncounterDatetime(), encounter.getLocation());
				obs.setValueText(number);
				encounter.addObs(obs);
			}
		} 
	...
	public Concept getMDRStatus() {
		Obs obs = MdrtbUtil.getObsFromEncounter(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.MDRTB_STATUS), encounter);
	...
	public void setMDRStatus(Concept status) {
		...
		if (obs == null || obs.getValueCoded() == null || !obs.getValueCoded().equals(status)) {
			...
			// now create the new Obs and add it to the encounter	
			if(status != null) {
				obs = new Obs (encounter.getPatient(), Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.MDRTB_STATUS), encounter.getEncounterDatetime(), encounter.getLocation());
	...
	public Date getConfirmationDate() {
		Obs obs = MdrtbUtil.getObsFromEncounter(Context.getService(MdrtbService.class).getConcept(TbConcepts.CONFIRMATION_DATE), encounter);
	...
	public void setConfirmationDate(Date date) {
		...
		if (obs == null || obs.getValueDatetime() == null || !obs.getValueDatetime().equals(date)) {
			...
			// now create the new Obs and add it to the encounter	
			if(date != null) {
				obs = new Obs (encounter.getPatient(), Context.getService(MdrtbService.class).getConcept(TbConcepts.CONFIRMATION_DATE), encounter.getEncounterDatetime(), encounter.getLocation());
